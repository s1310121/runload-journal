...
export const A9_APP_OVERLAY_VERSION = "runload-v2.6c1-condition-overlay-v1.0";
export const A9_APP_AUTHORITY_VERSION = "RunLoad-V2.6C1-Corrective-Scientific-Authority-20260823";
...
export function buildA9AppReviewInput(record = {}, options = {}) {
  ...
  return {
    ...
    gradeInputMode: upper(course.gradeInputMode || (Array.isArray(course.sections) && course.sections.length ? "SECTIONS" : grade.gradeKnown && grade.gradePercent === 0 ? "FLAT" : "UNKNOWN")),
...
function lineageForRegion(region = {}, { runningPhase = false, mixedRunningPhase = false, gradeInputMode = "UNKNOWN", contextInputIds = [] } = {}) {
  const numeric = new Set();
  const routeGate = new Set(["RL-IN-003", "RL-IN-017", "RL-IN-018", "RL-IN-032", "RL-IN-040"]);
  const aggregation = new Set(["RL-IN-041"]);
  const contextInputs = new Set(contextInputIds);
  const derived = [];
  if (runningPhase) {
  ...
  const mode = String(gradeInputMode || "UNKNOWN").toUpperCase();
  if (mode === "SUMMARY") {
    aggregation.add("RL-IN-033"); aggregation.add("RL-IN-034"); aggregation.add("RL-DV-035");
    derived.push({ derivedInputId: "RL-DV-035", dependsOnInputIds: ["RL-IN-033", "RL-IN-034"], meaning: "flatSharePercent=100-uphillSharePercent-downhillSharePercent" });
  ...
  const used = new Set([...numeric, ...routeGate, ...aggregation, ...contextInputs]);
  return Object.freeze({
    ...
    contextInputIds: Object.freeze([...contextInputs].sort()),
...
function explicitContextInputIds(record = {}) {
  const personal = record.personalContext || {};
  const ids = [];
  if (Object.prototype.hasOwnProperty.call(personal, "shoeType") && personal.shoeType != null) ids.push("RL-IN-072");
  if (Object.prototype.hasOwnProperty.call(personal, "shoeSoftness") && personal.shoeSoftness != null) ids.push("RL-IN-073");
  if (Object.prototype.hasOwnProperty.call(personal, "footPlacement") && personal.footPlacement != null) ids.push("RL-IN-080");
  return ids;
}
...
function overlayRow(row, a9Region, context = {}) {
  ...
  const flags = { runningPhase, mixedRunningPhase, gradeInputMode: context.gradeInputMode || "UNKNOWN", contextInputIds: context.contextInputIds || [] };
  ...
  return {
    ...
    a9ConditionEvidence: {
      ...
      contextSourceIds: clone(a9Region.contextSourceIds || []),
...
function failClosedWithStatus(calculationOutput, status, detail = {}) {
  ...
  next.legacyBaseAuthorityVersion = calculationOutput.authorityVersion || null;
  next.legacyBaseParameterSetVersion = calculationOutput.parameterSetVersion || null;
...
export function applyA9ConditionOverlay(calculationOutput = {}, record = {}, options = {}) {
  ...
  next.regions = (next.regions || []).map((row) => {
    ...
    const overlaid = overlayRow(row, a9Region, {
      ...
      gradeInputMode: input.gradeInputMode || "UNKNOWN",
      contextInputIds: explicitContextInputIds(record),
    });
