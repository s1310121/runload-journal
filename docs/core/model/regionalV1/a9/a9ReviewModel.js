...
export const A9_REVIEW_MODEL_VERSION="RunLoad-V2.6C1-12region-review-model-v1.0";
...
function routeFamily(regionId,result){
  if(result?.a9CanonicalFamilyId)return result.a9CanonicalFamilyId;
...
function buildSections(input,speedMps){
  if(Array.isArray(input.sections)&&input.sections.length){
    ...
    return explicit.map((section,index)=>({
      ...
      cadenceSpm:finitePositive(section.cadenceSpm)??(homogeneousWholeRun?finitePositive(input.cadenceSpm):null),
...
export function calculateA9RegionalReview(input={},options={}){
  ...
  const speedOutOfDomain=sections.find((section)=>Number.isFinite(section.speedMps)&&(section.speedMps<1.50-1e-12||section.speedMps>5.00+1e-12));
  if(speedOutOfDomain)return {ok:false,error:{code:"SPEED_OUT_OF_MODEL_USE_DOMAIN",message:"running speed is outside the 1.50–5.00 m/s research model-use domain",sectionId:speedOutOfDomain.sectionId,speedMps:speedOutOfDomain.speedMps}};
  ...
  const regions=REGIONS.map(region=>{
    const evaluated=sections.map(section=>{
      ...
      const context={
        ...
        cadenceReference:input.cadenceReference??null,
    ...
    const contextSourceIds=[...new Set(supported.flatMap(item=>item.result?.a9ContextSourceIds??[]))].sort();
    ...
    return {
      ...
      activeRouteIds,sourceIds,contextSourceIds,
      ...
      canonicalFamilyId:compatible?(supported[0]?.result?.a9CanonicalFamilyId??null):null,
      cadenceDisposition:region.id==="BA-DISP-019"?(supported[0]?.result?.a9Cadence??{state:"BUILDING_REFERENCE",numericEffectApplied:false}):null,
      sectionAudit:evaluated.map(item=>({sectionId:item.section.sectionId,distanceKm:item.section.distanceKm,speedMps:item.section.speedMps??null,gradePercent:signedGrade(item.section),supported:item.supported,family:item.family,canonicalFamilyId:item.supported?(item.result?.a9CanonicalFamilyId??null):null,semanticCompatibilityKey:item.semanticKey??null,conditionConstructId:item.supported?(item.result?.a9SemanticIdentity?.constructId??null):null,conditionReferenceDefinitionId:item.supported?(item.result?.a9SemanticIdentity?.referenceDefinitionId??null):null,evidenceOrigin:item.provenance?.evidenceOrigin??null,regionalMapping:item.provenance?.regionalMapping??null,ratio:item.supported?item.result.ratio:null,routeIds:item.supported?item.result.routes:[],supportTier:item.supported?(item.result.a9SupportTier??null):null,routeSignature:item.supported?(item.result.a9RouteSignature??null):null,uncertaintyClass:item.supported?(item.result.a9UncertaintyClass??null):null,surfaceDispositions:item.supported?(item.result?.a9SurfaceDispositions??[]):[],surfaceModifier:item.supported?(item.result?.a9SurfaceModifier??null):null,cadenceDisposition:item.supported?(item.result?.a9Cadence??null):null,contextSourceIds:item.supported?(item.result?.a9ContextSourceIds??[]):[],reason:item.reason})) ,
    };
  });
