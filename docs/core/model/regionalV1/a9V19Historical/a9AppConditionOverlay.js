...
export const A9_APP_OVERLAY_VERSION = "runload-a9-fcr-v1.9-condition-overlay-v1.0";
export const A9_APP_AUTHORITY_VERSION = "RunLoad-A9-FCR-Total-Audit-20260821-V2.0";
...
function inputIdsForRoute(region = {}, { runningPhase = false, mixedRunningPhase = false } = {}) {
  const ids = ["RL-IN-003", "RL-IN-017", "RL-IN-032", "RL-IN-039", "RL-IN-040", "RL-IN-041"];
  if (runningPhase) ids.push(RW_DISTANCE_INPUT_ID, RW_DURATION_INPUT_ID);
  else ids.push("RL-IN-011", "RL-DV-019");
  if (mixedRunningPhase) ids.push(RW_SECTIONS_INPUT_ID);
  if ((region.activeRouteIds || []).some((id) => String(id).includes("GRADE") || String(id).includes("RICE") || String(id).includes("MIXGRADE"))) ids.push("RL-IN-036", "RL-IN-037");
  return unique(ids);
}
...
function overlayRow(row, a9Region, context = {}) {
  ...
  const used = unique(inputIdsForRoute(a9Region, flags));
...
export function applyA9ConditionOverlay(calculationOutput = {}, record = {}) {
  ...
  if (!review.ok && runningPhase && review.error?.code === "RUN_WALK_PHASE_TO_SECTION_MAPPING_REQUIRED") {
    ...
    const value = passthroughWithStatus(calculationOutput, status, { detailCode });
    return { ok: true, value, audit: { appliedRegionIds: [], a9Review: null, status } };
  }
  if (!review.ok && ["TREADMILL_OUTDOOR_MIX_FORBIDDEN","GRADE_OUT_OF_MODEL_USE_DOMAIN"].includes(review.error?.code)) {
    const value = passthroughWithStatus(calculationOutput, review.error.code, { modelUseDomainFailure: clone(review.error) });
