...
const SCREEN_HINTS = Object.freeze({
  ...
  result: "走行全体の比較用推定値と12部位の指数を別々に確認します。",
  "body-part-detail": "選んだ部位の指数、関連する一般知識、本人の身体記録を確認します。",
...
function renderFirstUse(currentScreen) {
  ...
  const steps = [
    ...
    ["12部位の数値を確認", "各部位の指数を、その部位自身の基準100と比べます。ほかの部位とのランキングではありません。"],
    ["本人の感覚を別に残す", "身体記録は数値結果と分けて保存し、本人が入力した内容として見返します。"],
    ["表示を区別", "走行全体の比較用推定値、部位ごとの負荷傾向指数、履歴、本人の記録ノートを目的別に分けます。"],
  ];
...
function renderRecordGuide() {
  return `<div class="guide-lead"><p>必須項目を前面に置き、任意項目は必要なときだけ別画面で追加します。</p></div><div class="guide-card-grid">
    <article><h3>必須の走行量</h3><p>走行日には距離と実走時間が必要です。休養日は走行による12部位の数値を表示しません。</p></article>
...
function renderResultGuide() {
  return `<div class="guide-lead"><p>結果画面では、目的の違う情報を分けて表示します。</p></div><ol class="guide-reading-order">
    <li><strong>12部位の負荷傾向指数</strong><span>各部位を、その部位自身の表示上の基準100と比べます。</span></li>
    <li><strong>走行全体の比較用推定値</strong><span>今回の走行量を、比較できる本人の過去記録と見比べます。12部位の指数とは別の見方です。</span></li>
    <li><strong>基準との差</strong><span>指数128なら基準から+28ポイントです。実際の力が28%増えたという意味ではありません。</span></li>
  ...
  </ol><div class="guide-note"><h3>比べるときの注意</h3><p>12部位の値を同じ単位の順位として読みません。履歴では、同じ部位・同じ基準など、同じ意味で比べられる記録だけを線で結びます。</p></div>`;
}
...
function renderRecordsGuide() {
  return `<div class="guide-lead"><p>履歴と記録ノートは、保存する情報の役割を分けています。</p></div><div class="guide-card-grid">
    ...
    <article><h3>記録ノート</h3><p>本人が選んだテーマと1行を保存します。距離や指数を自動複製せず、元の結果へリンクします。</p></article>
...
function renderSafetyGuide() {
  return `<div class="guide-lead"><p>RunLoadは走行条件を整理して振り返るためのアプリで、身体や障害を判定するものではありません。</p></div><div class="guide-scope-grid"><article><h3>支援すること</h3><ul><li>走行・休養・コース事実の保存</li><li>12部位の基準100指数</li><li>本人入力と数値結果の区別</li><li>履歴、ノート、相談準備</li></ul></article><article><h3>主張しないこと</h3><ul><li>実際の筋・腱・関節力</li><li>診断、原因、危険度、傷害確率</li><li>走行可否、受診要否、個別処方</li><li>部位間の物理的な大小順位</li></ul></article></div><div class="guide-card-grid">
