...
export const RESULT_DISPLAY_MODE_OPTIONS = Object.freeze([
  Object.freeze({ value: "standard", label: "標準（入力から読む）", description: "結果画面は、今日の入力内容を先に表示し、その後で走行全体の比較用推定値と部位ごとの負荷傾向指数を表示します。" }),
...
export const REGIONAL_RESULT_INITIAL_VIEW_OPTIONS = Object.freeze([
  Object.freeze({ value: "focus", label: "基準との差がある部位を先に表示", description: "各部位自身の基準100との差が大きい部位を最大4部位表示します。危険度や部位間順位ではありません。" }),
  Object.freeze({ value: "all", label: "全12部位を先に表示", description: "結果画面を開いたときから、12部位を解剖学的な順序で表示します。" }),
  Object.freeze({ value: "remember", label: "前回の切替を引き継ぐ", description: "結果画面で最後に選んだ「基準との差がある部位／全12部位」を次回も使います。" }),
]);
...
export const REGIONAL_PREVIOUS_COMPARISON_OPTIONS = Object.freeze([
  ...
  Object.freeze({ value: "hide", label: "表示しない", description: "各部位の基準100との比較だけを表示します。保存結果や履歴は変更しません。" }),
]);
