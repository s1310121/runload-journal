...
function renderLocalStorageSection(overview) {
  return `<section class="privacy-section" aria-labelledby="privacy-local-title"><div class="section-heading"><p>端末内保存</p><h2 id="privacy-local-title">このブラウザーに保存するもの</h2><p>RunLoadは、記録を同じ端末・同じブラウザーで見返せるように、次の情報をブラウザーの保存領域へ残します。</p></div>${renderStorageGroups(overview.storageGroups)}<aside class="privacy-note"><p><strong>ブラウザーやOSでサイトデータを削除すると、端末内の記録も失われることがあります。</strong></p><p>重要な記録は、必要なときに設定画面からバックアップしてください。</p></aside></section>`;
}

function renderTemporarySection() {
  return `<section class="privacy-section" aria-labelledby="privacy-temporary-title"><div class="section-heading"><p>一時保持</p><h2 id="privacy-temporary-title">入力関連画面を往復するときの一時データ</h2></div><div class="privacy-explanation-card"><p>コース、本人入力、シューズの補助画面を開いて入力画面へ戻る間は、入力途中の内容を一時的に保持します。</p><p>入力関連画面の外へ移動したとき、記録を保存したとき、または「このアプリの端末内データを削除」を実行したときに消去します。ブラウザーの動作によっては、タブを閉じるまで残る場合があります。</p></div></section>`;
}

function renderNoAutomaticCollectionSection() {
  return `<section class="privacy-section" aria-labelledby="privacy-not-collected-title"><div class="section-heading"><p>自動取得・送信</p><h2 id="privacy-not-collected-title">RunLoadが自動取得・送信しないもの</h2></div><ul class="privacy-check-list"><li>GPS位置情報、外部アカウント、地図画像、標高グラフ</li><li>外部の解析サービスへの記録送信</li><li>相談メモの自動送信・外部保存</li><li>バックグラウンドでの記録アップロード</li></ul><p class="source-boundary">RunLoadは、利用者が入力・保存した内容を外部サーバーへ自動送信しません。オフラインで画面を開きやすくするために保存されるアプリの表示データにも、利用者の記録を送信する機能はありません。</p></section>`;
}

function renderExternalLinksSection() {
  return `<section class="privacy-section" aria-labelledby="privacy-external-title"><div class="section-heading"><p>外部リンク</p><h2 id="privacy-external-title">別のサイトや電話機能を開く場合</h2></div><div class="privacy-explanation-grid"><article><h3>外部サイト</h3><p>地図、公開論文、公的案内などのリンクを選ぶと、RunLoadとは別のサイトが新しいタブで開きます。RunLoadの端末内記録をURLへ自動追加したり、外部サイトへ送信したりしません。</p><p>開いた先では、そのサイトの通信・Cookie・プライバシー方針が適用されます。</p></article><article><h3>電話リンク</h3><p>119や#7119の電話リンクを選ぶと、対応端末では電話機能が開きます。RunLoadから自動発信せず、発信操作は端末側で行います。RunLoadは通話内容、通話履歴、電話番号を取得・保存しません。</p></article></div></section>`;
}

function renderBackupSection() {
  return `<section class="privacy-section" aria-labelledby="privacy-backup-title"><div class="section-heading"><p>バックアップ</p><h2 id="privacy-backup-title">JSONファイルに含まれるもの</h2></div><div class="privacy-explanation-card"><p>「バックアップを保存」を選んだときだけ、走行・休養記録、保存済み結果、本人入力、予定、記録ノート、任意プロフィール、表示設定、下書き、保存コースをJSONファイルへ書き出します。</p><p><strong>このファイルは平文JSONで、RunLoadによるパスワード保護や暗号化はありません。</strong> 内容を読める人へ共有しない場所で保管してください。</p><p>作成したファイルは端末のダウンロード先など、ブラウザーが選んだ場所に保存されます。RunLoadが自動でクラウドへアップロードすることはありません。</p></div></section>`;
}
