...
export const COLUMN_ARTICLES = Object.freeze([
  ...
  article({
    ...
    title: "12部位の条件応答をどう読むか",
    category: "結果の読み方",
    tags: ["部位ごとの条件応答", "共通走行量", "12部位", "基準100", "身体図"],
    lead: "根拠付きで数値化できる走行条件への応答を、各部位自身の基準100と比較します。共通走行量は別に表示します。",
    summary: "条件応答を数値化できる部位では100との差を見ます。数値がない部位を100で補わず、別の部位との順位も表しません。",
    body: [
      "坂の向き、走る速さ、足の運び方が変わると、関節の動き、筋肉の働き、足裏の圧のかかり方なども変わることが研究で報告されています。変化の仕方は部位ごとに同じではありません。",
      "この画面では、根拠付きで数値化できる走行条件への応答を、各部位自身の基準100と比べます。たとえば128なら、その部位の条件応答の基準より28ポイント上です。膝の128と足裏の120を比べて、膝の負荷が大きいと判断することはできません。",
      "距離・歩数・接触回数などの共通走行量は条件応答と分けて確認します。条件応答を数値化できない部位では、共通走行量や基準100を代わりの条件応答として入れません。",
    ],
    practicePoints: [
      "条件応答の数値がある部位では、その部位自身の基準100との差を確認する。",
      "次に、坂・ペース・路面などの条件と、別欄の共通走行量を分けて見る。",
      "過去比較があるときは、同じ意味で比べられる記録かを確認する。",
    ],
    caution: "この条件応答は実測値、傷害確率、危険度、走行可否、部位間の物理的順位ではありません。基準100は安全値・正常値・初心者平均でもありません。",
  ...
  article({
    ...
    lead: "12部位の条件応答表示と、本人が選ぶ詳細な身体記録は目的が異なります。",
    ...
    body: [
      ...
      "本人の身体記録では、感じた場所、左右、程度、気づいた時点を残せます。こちらは本人の感覚を記録するもので、12部位の条件応答とは分けて表示します。",
      "2つの表示が同じ方向でも違っていても、それだけで原因は分かりません。気になったことがあれば、走った条件と本人の感覚を分けて残すと、あとで振り返りやすくなります。",
    ],
    practicePoints: [
      "身体図では、条件応答を数値化できる部位だけ、その部位自身の基準100との差を見る。",
  ...
  article({
    ...
    lead: "同じ部位で、同じ意味の条件応答として直接比べられる以前の記録がある場合だけ表示します。",
    summary: "今回の条件応答を、同じ部位・同じ基準・同じ条件経路で比べられる直近の過去記録とだけ見比べます。",
    body: [
      "部位の条件応答は、同じ部位であっても速度系列と勾配系列など、使った研究条件の経路が違えば直接比較しません。部位、基準、条件経路がそろう記録だけを比較候補にします。",
      "直接比べられる以前の記録があるときは、その中で直近の1件と今回の差を表示します。比べられる記録がなければ、無理に差を作らず理由を示します。",
      "この比較は正常値や理想値との比較ではありません。今回と以前の条件応答を同じ意味の範囲で振り返るための参考です。",
    ],
    practicePoints: [
      "条件応答を数値化できる場合は、その部位自身の基準100との関係も確認する。",
  ...
  article({
    ...
    body: [
      ...
      "部位別の履歴では、選んだ部位・基準・条件経路の意味をそろえます。速度系列と勾配系列など、直接比べられない条件応答を途中でつなぎません。",
  ...
  article({
    ...
    body: [
      ...
      "研究ごとに調べた部位や条件が違うため、12部位は同じ単位で測った順位ではありません。条件応答を数値化できる部位では、その部位自身の基準100との差と、その日の走行条件を一緒に見ます。数値化できない部位は数値なしのまま確認します。",
    ],
    practicePoints: [
